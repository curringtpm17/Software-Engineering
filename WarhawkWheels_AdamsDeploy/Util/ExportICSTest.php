...
ATTENDEE;CN="<?=$row["lastName"] . ", " . $row["firstName"]?>";ROLE=REQ-PARTICIPANT;PART-STAT=NEEDS-ACTION;RSVP=TRUE:mailto:<?=$row["Email"]?> 
DTEND;TZID=America/Chicago:<?= relDateToCal($reldate, endTime($row["startTime"])) ?> 
UID:<?= uniqid() ?> 
...
DTSTART;TZID=America/Chicago:<?= relDateToCal($reldate, $row["startTime"]) ?> 
END:VEVENT
BEGIN:VALARM
ACTION:DISPLAY
DESCRIPTION:<?= escapeString($description) ?> 
