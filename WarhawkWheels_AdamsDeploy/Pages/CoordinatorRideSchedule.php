...
//echo "<br>Showing rides for " . $_SESSION["ShowThisDay"] . "s.<br>";
?>
<br>
...
<form action=<?php echo htmlspecialchars("index.php"); ?> method="post">
    ...
    Show rides for: 
    <input type="submit" name="Monday" value="Monday" <?php if (isset($_SESSION["ShowThisDay"]) && $_SESSION["ShowThisDay"] == "Monday") {
    echo "disabled class=\"selectedDay\"";
} ?>>
    <input type="submit" name="Tuesday" value="Tuesday" <?php if (isset($_SESSION["ShowThisDay"]) && $_SESSION["ShowThisDay"] == "Tuesday") {
    echo "disabled class=\"selectedDay\"";
} ?>>
    <input type="submit" name="Wednesday" value="Wednesday" <?php if (isset($_SESSION["ShowThisDay"]) && $_SESSION["ShowThisDay"] == "Wednesday") {
    echo "disabled class=\"selectedDay\"";
} ?>>
    <input type="submit" name="Thursday" value="Thursday" <?php if (isset($_SESSION["ShowThisDay"]) && $_SESSION["ShowThisDay"] == "Thursday") {
    echo "disabled class=\"selectedDay\"";
} ?>>
    <input type="submit" name="Friday" value="Friday" <?php if (isset($_SESSION["ShowThisDay"]) && $_SESSION["ShowThisDay"] == "Friday") {
    echo "disabled class=\"selectedDay\"";
} ?>>
    <br>
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo "<tr>";
                echo "	<td>" . convertToTwelveHr($row["startTime"]) . "</td><td>" . $row["Name"] . "</td><td>" . $row["source"] . "</td><td>" . $row["destination"] . "</td><td>" . $row["vanId"] . "</td><td>" . "<form class=\"hiddenForm\" action=\"index.php\" method=\"post\">" . "<input type=\"submit\" value=\"Inactivate Ride\"  name=\"InactivateRide" . $row["rideId"] . "\"/>" . "<input type=\"submit\" value=\"Edit Ride\"  name=\"EditRide" . $row["rideId"] . "\"/>" . "</form>" . "</td>";
                echo "</tr>";
            }
        } else {
        ...
        $queryResult = "Coordinator Ride Active Error: " . $e->getMessage();
    }
    ?>
</table>
...
<form class="hiddenForm" name="loginForm" action=<?php echo htmlspecialchars("Util/ExportICS.php"); ?> method="post">
    <input type="submit" Value="Export ICS File"/>
</form>
<br><br>